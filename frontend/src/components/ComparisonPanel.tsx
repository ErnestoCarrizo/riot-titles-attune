...
function HoverTooltip({ text, children }: { text: string; children: ReactNode }) {
  ...
  useLayoutEffect(() => {
    ...
    const top = below + tooltip.height <= window.innerHeight - gutter ? below : Math.max(gutter, anchor.top - tooltip.height - 8);
    setPosition({ left, top });
  }, [visible]);

  return (
    <>
      <span ref={anchorRef} className="comparison-row__title" onMouseEnter={() => setVisible(true)} onMouseLeave={() => setVisible(false)} onFocus={() => setVisible(true)} onBlur={() => setVisible(false)} tabIndex={0}>
        {children}
      </span>
      {visible ? createPortal(<span ref={tooltipRef} className="comparison-tooltip" style={{ left: position.left, top: position.top }}>{text}</span>, document.body) : null}
    </>
  );
}
    ...
    <section className="comparison-panel" aria-labelledby="comparison-title">
      ...
      <div className="comparison-list" role="list" aria-label={contextual ? 'Comparación de nodos' : 'Comparación de títulos'}>
        {!contextual ? primary.titles.map((title) => {
          ...
          return <button className={`comparison-row comparison-row--node ${canAdvance ? '' : 'comparison-row--leaf'}`} type="button" role="listitem" key={node.challengeId} onClick={() => { if (canAdvance) onAdvanceNode(String(node.challengeId)); }} aria-label={`${node.challengeName}. ${canAdvance ? 'Avanzar a sus hijos' : 'Objetivo final'}`}><HoverTooltip text={tooltip}><strong>{node.challengeName}</strong><small>{differenceLabel(primaryPercent, secondaryPercent, primary.player.gameName)}</small></HoverTooltip><Score percent={primaryPercent} status={node.status} tone="cyan" owner={primary.player.gameName} /> <Score percent={secondaryPercent} status={other?.status} tone="orange" owner={secondary.player.gameName} />{canAdvance ? <Icon name="chevron-right" size={17} /> : <span aria-hidden="true" />}</button>;
        }) : contextual && !treeLoading && focusedTitle ? <button className="comparison-row is-selected" type="button" role="listitem" onClick={() => onSelectTitle(focusedTitle.titleId)}><HoverTooltip text={titleTooltip(focusedTitle)}><strong>{focusedTitle.titleName}</strong><small>{differenceLabel(titlePercent(focusedTitle), titlePercent(focusedSecondaryTitle), primary.player.gameName)}</small></HoverTooltip><Score percent={titlePercent(focusedTitle)} status={focusedTitle.status} tone="cyan" owner={primary.player.gameName} /><Score percent={titlePercent(focusedSecondaryTitle)} status={focusedSecondaryTitle?.status} tone="orange" owner={secondary.player.gameName} /><span aria-hidden="true" /></button> : null}
      </div>
    </section>
  );
}
